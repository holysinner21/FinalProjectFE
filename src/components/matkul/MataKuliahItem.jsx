...
export default function MataKuliahItem({ matkul, reload }) { 
    ...
    async function executeDelete() {
        setIsDeleting(true);
        console.log(`[DELETE] Memulai penghapusan Matkul ID: ${matkul.id}`);

        try {
            const res = await fetch(`${API_BASE_URL}/${matkul.id}`, {
                method: "DELETE",
            });
            ...
            console.log(`[SUCCESS] Mata kuliah "${matkul.nama}" berhasil dihapus.`);
            if (reload) reload();
